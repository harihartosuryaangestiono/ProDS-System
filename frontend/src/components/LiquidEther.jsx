...
export default function LiquidEther({
  ...
  useEffect(() => {
    ...
    class MouseClass {
      ...
      onTouchEnd() {
        this.isHoverInside = false;
        // Reset user control saat touch end agar auto demo langsung aktif
        setTimeout(() => {
      ...
      onDocumentLeave() {
        this.isHoverInside = false;
        // Reset user control saat mouse keluar agar auto demo langsung aktif
        if (!this.isHoverInside) {
          // Beri sedikit delay untuk smooth transition
          setTimeout(() => {
            if (!this.isHoverInside) {
              this.hasUserControl = false;
            }
    ...
    class AutoDriver {
      constructor(mouse, manager, opts) {
        ...
        this.pickNewTarget();
        // Inisialisasi: jika enabled dan tidak ada hover, langsung aktifkan
        if (this.enabled && !this.mouse.isHoverInside) {
          this.active = true;
      ...
      update() {
        ...
        // Jika mouse sedang hover, hentikan auto demo
        if (this.mouse.isHoverInside) {
          if (this.active) this.forceStop();
          return;
        }
        
        // Jika mouse tidak hover dan tidak ada user control, langsung aktifkan auto demo
        ...
        // Jika ada user control sebelumnya, tunggu resume delay
        else if (this.mouse.hasUserControl && idle >= this.resumeDelay) {
          if (!this.active) {
            this.active = true;
            this.current.copy(this.mouse.coords);
            this.lastTime = now;
        ...
        // Masih dalam resume delay setelah interaksi user
        else {
          if (this.active) this.forceStop();
          return;
        }
