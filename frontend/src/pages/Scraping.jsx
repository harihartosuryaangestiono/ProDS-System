...
import { Play, User, Lock, Database, AlertCircle, CheckCircle, Clock, Target, Globe, FileText, Loader, Square, Bug, ChevronDown, ChevronUp, Copy, Check, XCircle } from 'lucide-react';
import Layout from '../components/Layout';

const ScrapingDashboard = () => {
  const [activeTab, setActiveTab] = useState('sinta-dosen');
  const [credentials, setCredentials] = useState({ username: '', password: '' });
  const [gsConfig, setGsConfig] = useState({ maxAuthors: 20, scrapeFromBeginning: false });
  ...
  // Ref untuk akses currentJobId terbaru di dalam interval callback
  ...
  // ── Helper: hentikan polling ─────────────────────────────────────────────
  const stopPolling = () => {
    if (pollIntervalRef.current) {
      clearInterval(pollIntervalRef.current);
      pollIntervalRef.current = null;
      addDebugLog('info', 'Polling dihentikan');
    }
  };

  // ── Helper: selesaikan job (apapun statusnya) ────────────────────────────
  const finalizeJob = (status, resultData) => {
    stopPolling();
    setIsLoading(false);
    setIsCancelling(false);
    setCurrentJobId(null);
    currentJobIdRef.current = null;
    setScrapingResults(resultData);
    addDebugLog('info', `Job finalized with status: ${status}`, resultData);
  };
  ...
  useEffect(() => {
    ...
    addDebugLog('info', 'Memulai polling job', { job_id: currentJobId });
    ...
    pollIntervalRef.current = setInterval(async () => {
      ...
      try {
        ...
        if (!response.ok) {
          addDebugLog('error', 'Job status request failed', {
        ...
        if (!data.success || !data.job) {
          addDebugLog('error', 'Response tidak valid', data);
          return;
        }

        const job = data.job;
        addDebugLog('info', `Status: ${job.status} — ${job.message || ''}`, { current: job.current, total: job.total });

        // Update progress bar untuk semua status aktif
        if (['running', 'starting', 'cancelling', 'login_in_progress', 'ready'].includes(job.status)) {
          setScrapingProgress({
            status: job.status,
            message: job.message || 'Sedang memproses...',
        ...
        if (job.status === 'completed') {
          ...
          if (!result || Object.keys(result).length === 0) {
            finalizeJob('completed_empty', {
              success: false,
              error: job.message || 'Scraper selesai tapi tidak ada data.',
              debugInfo: { job, result }
            });
          } else if (result.success === false) {
            finalizeJob('completed_failed', {
              success: false,
              error: result.error || result.message || job.message || 'Scraping gagal',
          ...
          // ── INI KUNCI PERBAIKAN: handle status cancelled ───────────────
        ...
        addDebugLog('error', 'Polling error', { message: error.message });
      }
    }, 2000);

    // Cleanup saat unmount atau job berubah
    return () => {
      stopPolling();
    };
  }, [currentJobId, isLoading]);

  // ── Credential & config handlers ─────────────────────────────────────────
  ...
  const startScraping = async (endpoint, payload) => {
    ...
    setScrapingProgress({
      ...
      currentCount: 0,
      targetCount: payload.target_dosen || payload.max_authors || payload.max_pages || 0
    });

    addDebugLog('info', 'Memulai scraping', { endpoint, payload });
    ...
    try {
      ...
      addDebugLog('info', 'Response diterima', { status: response.status, ok: response.ok });
      ...
      if (data.success && data.job_id) {
        ...
        addDebugLog('info', 'Job dimulai', { job_id: data.job_id });
        setScrapingProgress(prev => ({
          ...prev,
          job_id: data.job_id,
          message: data.message || 'Job dimulai, menunggu update...'
        }));
      } else if (data.success) {
        setScrapingResults({ success: true, message: data.message, summary: data.summary });
        setIsLoading(false);
      } else {
        addDebugLog('error', 'Gagal memulai scraping', data);
        setScrapingResults({
          success: false,
      ...
      addDebugLog('error', 'Network error', { message: error.message });
      setScrapingResults({
        success: false,
  ...
  const handleSintaDosen = async () => {
    if (!credentials.username || !credentials.password) { alert('Username dan password SINTA harus diisi'); return; }
  ...
  const handleGoogleScholar = async () => {
    ...
    setScrapingResults(null);
    setDebugLogs([]);
    setScrapingProgress({ status: 'starting', message: 'Memulai scraping Google Scholar...', currentCount: 0, targetCount: gsConfig.maxAuthors });
    ...
    try {
      const response = await fetch('/api/scraping/googlescholar/scrape', {
        ...
        body: JSON.stringify({ max_authors: gsConfig.maxAuthors, scrape_from_beginning: gsConfig.scrapeFromBeginning })
      });
      const data = await response.json();
  ...
  const handleGoogleScholarDosen = async () => {
    ...
    setScrapingResults(null);
    setDebugLogs([]);
    setScrapingProgress({ status: 'starting', message: 'Memulai scraping dosen Google Scholar...', currentCount: 0, targetCount: gsDosenConfig.maxPages });
    ...
    try {
      const response = await fetch('/api/scraping/googlescholar/dosen', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ max_pages: gsDosenConfig.maxPages, search_query: gsDosenConfig.searchQuery })
      });
      const data = await response.json();
  ...
  const cancelCurrentJob = async () => {
    ...
    addDebugLog('info', 'Mengirim permintaan cancel', { job_id: jobId });

    // Langsung update UI — jangan tunggu polling
    setScrapingProgress(prev => ({
      ...prev,
      status: 'cancelling',
      message: 'Menghentikan scraping, mohon tunggu...'
    }));
    ...
    try {
      ...
      if (!data.success) {
        // Cancel request gagal — kembalikan state
        setIsCancelling(false);
        setScrapingProgress(prev => ({ ...prev, status: 'running', message: 'Scraping berjalan...' }));
        alert(data.error || 'Gagal mengirim perintah cancel');
      }
      // Jika sukses: tunggu polling mendeteksi status 'cancelled' dari backend
      // finalizeJob() akan dipanggil otomatis saat polling menerima status 'cancelled'
  ...
  const getProgressPercentage = () => {
    if (!scrapingProgress?.targetCount) return 0;
    return Math.min(Math.round((scrapingProgress.currentCount / scrapingProgress.targetCount) * 100), 100);
  };

  const getStatusColor = () => {
    if (!scrapingProgress) return 'bg-gray-400';
    switch (scrapingProgress.status) {
      case 'starting': case 'running': case 'ready': case 'login_in_progress': return 'bg-blue-500';
      case 'cancelling': return 'bg-yellow-500';
  ...
  const TabButton = ({ id, label, icon: Icon, isActive, onClick }) => (
    <button
      ...
      className={`flex items-center px-4 py-2 text-sm font-medium rounded-lg transition-colors duration-200 ${
        isActive ? 'bg-blue-600 text-white shadow-md' : 'text-gray-600 hover:text-blue-600 hover:bg-blue-50'
      }`}
    >
  ...
  const CancelButton = () => {
    if (!isLoading || !currentJobId) return null;
    return (
      <button
  ...
  return (
    ...
    >
          ...
          <div className="bg-white rounded-lg shadow-sm p-4">
            <div className="flex items-center justify-between mb-2">
              <div className="flex items-center gap-2">
                {scrapingProgress.status === 'cancelling'
      ...
      {scrapingResults && (
        <div className={`mb-6 rounded-lg border ${
          ...
          <div className={`flex items-start p-4 ${
            !scrapingResults.success && !scrapingResults.cancelled ? 'border-b border-red-200' : ''
          }`}>
            {scrapingResults.cancelled
            ...
            <div className="flex-1 min-w-0">
              <h3 className={`text-lg font-semibold ${
                scrapingResults.cancelled ? 'text-gray-600'
                  : scrapingResults.success ? 'text-green-800' : 'text-red-800'
              }`}>
                {scrapingResults.cancelled ? 'Scraping Dibatalkan'
                  : scrapingResults.success ? 'Scraping Berhasil' : 'Scraping Gagal'}
              </h3>
              <p className={`text-sm mt-0.5 ${
                scrapingResults.cancelled ? 'text-gray-500'
                  : scrapingResults.success ? 'text-green-700' : 'text-red-700'
              }`}>
                {scrapingResults.cancelled
                  ? (scrapingResults.error || 'Scraping dihentikan oleh user.')
                  : scrapingResults.success
                    ? scrapingResults.message
                    : (scrapingResults.error || 'Terjadi error tidak diketahui')}
              </p>
              ...
              {/* Traceback */}
              {!scrapingResults.success && !scrapingResults.cancelled && scrapingResults.traceback && (
          ...
          {/* Debug Info (hanya saat error, bukan cancel) */}
          {!scrapingResults.success && !scrapingResults.cancelled && (
            <div className="p-4">
              <button
                      ...
                      <div className="bg-gray-900 rounded-md p-3 overflow-auto max-h-64 font-mono text-xs border border-gray-700">
                        {debugLogs.map((log, i) => (
                          <div key={i} className="mb-1">
                            ...
                            {log.data && <pre className="mt-0.5 ml-4 text-gray-400 whitespace-pre-wrap">{log.data}</pre>}
                          </div>
                        ))}
                      </div>
                    </div>
      ...
      {/* SINTA Credentials */}
      {!['google-scholar', 'gs-dosen'].includes(activeTab) && (
          ...
          <div>
            ...
            <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 mb-6">
              <div className="flex">
                ...
                <p className="ml-3 text-sm text-yellow-700">Proses ini dapat memakan waktu lama (hingga beberapa jam) tergantung target dosen.</p>
              </div>
            </div>
        ...
        {activeTab === 'gs-dosen' && (
          <div>
            ...
            <div className="bg-blue-50 border-l-4 border-blue-400 p-4 mb-6">
              <div className="flex">
                <AlertCircle className="h-5 w-5 text-blue-400 flex-shrink-0" />
                <p className="ml-3 text-sm text-blue-700">Scraping profil dosen langsung dari Google Scholar. Browser akan terbuka untuk scraping.</p>
              </div>
            </div>
            ...
            <div className="bg-yellow-50 border-l-4 border-yellow-400 p-4 mb-4">
              <div className="flex">
                ...
                <div className="ml-3">
                  ...
                  <p className="text-sm text-yellow-700">Scraping dilakukan secara parsial dengan membatasi jumlah author. Default maksimal: <strong>20</strong>.</p>
                </div>
              </div>
            </div>
            <div className="bg-blue-50 border-l-4 border-blue-400 p-4 mb-6">
